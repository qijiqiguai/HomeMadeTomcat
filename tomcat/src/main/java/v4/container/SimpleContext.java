...
import v4.container.http.Context;
import v4.container.http.InvokePatch;
import v4.container.http.Mapper;
import v4.container.valve.SimpleContextValve;
 ...
 *
 * @author wangqi
 * @date 2017/10/24 下午7:58
 *
 * Connector -> Processor -> Context -> Pipeline -> Valves -> BasicValves -> Mapper -> Context ->
 * Wrapper -> Pipeline -> Valves -> BasicValves -> Wrapper -> Loader -> Servlet -> process(Request, Response)
 */
public class SimpleContext implements Context, InvokePatch{
    Mapper mapper;
    private SimplePipeline pipeline;

    public SimpleContext(Mapper mapper){
        this.mapper = mapper;
    ...
    public Container map(HttpServletRequest request){
        return ((SimpleMapper) mapper).map(request, true);
    }
